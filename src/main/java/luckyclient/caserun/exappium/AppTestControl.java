		...
		if (restartstatus.indexOf("Status:true") > -1) {
			...
			if (buildstatus.indexOf("Status:true") > -1) {
				try {
					if("Android".equals(properties.getProperty("platformName"))){
						androiddriver = AppiumInitialization.setAndroidAppium();
						luckyclient.publicclass.LogUtil.APP.info("完成AndroidDriver初始化动作...APPIUM Server【http://"+properties.getProperty("appiumsever")+"/wd/hub】");
					}else if("IOS".equals(properties.getProperty("platformName"))){
						iosdriver=AppiumInitialization.setIosAppium();
						luckyclient.publicclass.LogUtil.APP.info("完成IOSDriver初始化动作...APPIUM Server【http://"+properties.getProperty("appiumsever")+"/wd/hub】");
					}
				} catch (Exception e) {
					luckyclient.publicclass.LogUtil.APP.error("初始化AppiumDriver出错 ！APPIUM Server【http://"+properties.getProperty("appiumsever")+"/wd/hub】", e);
					e.printStackTrace();
				}
				LogOperation caselog = new LogOperation();
				int[] tastcount = null;
