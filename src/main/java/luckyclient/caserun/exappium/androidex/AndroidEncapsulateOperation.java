...
public class AndroidEncapsulateOperation {
	...
	public static String objectOperation(AndroidDriver<AndroidElement> appium,AndroidElement ae,String operation,String operationValue,String property,String propertyValue) throws Exception{
		String result = "";
		...
		case "longpresselement":
			...
			result = "longpresselement在屏幕指定元素上按住" + operationValue + "秒...【对象定位属性:"+property+"; 定位属性值:"+propertyValue+"】";
	...
	public static String driverOperation(AndroidDriver<AndroidElement> appium,String operation,String operationValue) throws Exception{
		String result = "";
		AndroidTouchAction action = new AndroidTouchAction(appium);
		// 处理页面对象操作
		switch (operation) {
			...
			if(null!=movexy[0]&&"".equals(movexy[0])){
				...
				for (int movexyi = 1; movexyi < movexy.length; movexyi++) {
					if (null != movexy[movexyi]&&"".equals(movexy[movexyi])) {
						...
						if(movexyi==1){
							action=action.press(PointOption.point(Integer.valueOf(startxy[0]), Integer.valueOf(startxy[1])))
									.waitAction(WaitOptions.waitOptions(Duration.ofNanos(1000)))
									.moveTo(PointOption.point(Integer.valueOf(endxy[0]), Integer.valueOf(endxy[1])));
							luckyclient.publicclass.LogUtil.APP.info("从开始坐标【"+startxy[0]+","+startxy[1]+"】拖动至坐标【"+endxy[0]+","+endxy[1]+"】");
						}else{
							action=action.waitAction(WaitOptions.waitOptions(Duration.ofNanos(1000)))
									.moveTo(PointOption.point(Integer.valueOf(endxy[0]), Integer.valueOf(endxy[1])));
							luckyclient.publicclass.LogUtil.APP.info("第"+movexyi+"次拖动至坐标【"+endxy[0]+","+endxy[1]+"】");
						}
					}else{
