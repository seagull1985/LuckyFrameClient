...
public class TestCaseExecution{	
		...
		List<ProjectCasesteps> steps=GetServerAPI.getStepsbycaseid(testcaseob.getId());
		    ...
		    for(int i=0;i<steps.size();i++){		    	
			    	...
			    	for(int j=0;j<casescript.size()-4;j++){		    		
			    		...
			    		if(casescript.get("FunctionParams"+(j+1)).indexOf("@")>-1){                        //如果存在传参，进行处理
			    			...
			    			if(sumvariable==1){
			    				...
			    				caselog.UpdateCaseLogDetail(testCaseExternalId, taskid, "你好像在一个参数中引用了超过2个以上的变量哦！我处理不过来啦！", "error",String.valueOf(i+1));
			    			}
			    			...
			    			if(keyexistidentity == 1){
					    		...
					    		caselog.UpdateCaseLogDetail(testCaseExternalId, taskid, "解析包名："+packagename+" 方法名："+functionname
			    				...
			    				caselog.UpdateCaseLogDetail(testCaseExternalId, taskid, "没有找到你要的变量哦，再找下吧！第二变量名称是："+uservariable+"，第"
			    ...
			    try{
		    		...
		    		caselog.UpdateCaseLogDetail(testCaseExternalId, taskid, "开始调用方法："+functionname+" .....", "info",String.valueOf(i+1));
			    	if(expectedresults.length()>2 && expectedresults.substring(0, 2).indexOf("$=")>-1){                             //把预期结果前两个字符判断是否是要把结果存入变量
				    	...
				    	if(testnote.indexOf(expectedresults.substring(2))>-1){
				    		...
				    		caselog.UpdateCaseLogDetail(testCaseExternalId, taskid, "用例执行结果是："+testnote+"，与预期结果匹配成功！", "info",String.valueOf(i+1));
				    	}else{
				    		setresult = 1;
				    		luckyclient.publicclass.LogUtil.APP.error("用例第"+(i+1)+"步执行结果与预期结果匹配失败！");
				    		caselog.UpdateCaseLogDetail(testCaseExternalId, taskid, "用例第"+(i+1)+"步执行结果与预期结果匹配失败！预期结果："+expectedresults+"      测试结果："+testnote, "error",String.valueOf(i+1));
				    	...
				    	if(expectedresults.equals(testnote)){
				    		...
				    		setresult = 1;
				    		luckyclient.publicclass.LogUtil.APP.error("用例第"+(i+1)+"步执行结果与预期结果匹配失败！");
				    		caselog.UpdateCaseLogDetail(testCaseExternalId, taskid, "用例第"+(i+1)+"步执行结果与预期结果匹配失败！预期结果："
			    	...
			    	caselog.UpdateCaseLogDetail(testCaseExternalId, taskid, "调用方法过程出错，方法名："+functionname+" 请重新检查脚本方法名称以及参数！", "error",String.valueOf(i+1));
		    ...
		    if(testnote.indexOf("CallCase调用出错！")<=-1&&testnote.indexOf("解析出错啦！")<=-1){
		    	...
		    	caselog.UpdateCaseLogDetail(testCaseExternalId, taskid, "解析成功，并成功调用用例中方法，请继续查看执行结果！", "info","SETCASERESULT...");
	 ...
	 */
	@SuppressWarnings("static-access")
	protected static String OneCaseExecuteForWebDriver(String testCaseExternalId,int version,String taskid){
		Map<String,String> variable = new HashMap<String,String>();
		String packagename =null;
		String functionname = null;
		String expectedresults = null;
		...
		if(testcaseob.getExecutionType()==ExecutionType.AUTOMATED){
		    ...
		    for(int i=0;i<testcaseob.getSteps().size();i++){		    	
		    	Map<String,String> casescript = InterfaceAnalyticTestLinkCase.AnalyticCaseStep(testcaseob, i+1,taskid);    //解析单个步骤中的脚本
		    	packagename = casescript.get("PackageName").toString();
		    	functionname = casescript.get("FunctionName").toString();
		    	//用例名称解析出现异常或是单个步骤参数解析异常
		    	if(functionname.indexOf("解析异常")>-1||k==1){
